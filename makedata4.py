...
def process_data():
    ...
    output_file = "data/路线推荐.jsonl"
    
    # 读取所有数据并筛选包含"路线推荐"的数据
    filtered_data = []
    with open(input_file, 'r', encoding='utf-8') as f:
        for line in f:
            data = json.loads(line.strip())
            if "路线推荐" in data.get("response", ""):
                filtered_data.append(data)
    
    print(f"找到 {len(filtered_data)} 条包含'路线推荐'的数据")
    ...
    for data in tqdm(filtered_data, desc="处理数据"):
        ...
        # 构建新的prompt
        new_prompt = f"""你是一个旅游专家，现在用户有一个旅游相关的问题，这个问题需要请求地图，获取某个起始地点的到目标地点的路线。
